...
bool CWeaponShotgun::HaveCartridgeInInventory		(u8 cnt)
{
	...
	if (m_pInventory)
	{
		...
		Current = smart_cast<CWeaponAmmo*>(m_pInventory->GetAny(*m_ammoTypes[m_ammoType]));
		...
		if(!Current )
		{
			for(u32 i = 0; i < m_ammoTypes.size(); ++i) 
			{
				...
				Current = smart_cast<CWeaponAmmo*>(m_pInventory->GetAny(*m_ammoTypes[i]));
				if(Current) 
				{ 
					m_ammoType = i; 
					break; 
	...
	return (Current!=NULL)&&(Current->m_boxCurr>=cnt);
}
