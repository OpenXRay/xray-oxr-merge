...
//		m.rot += (r - m.rot) * scaleFac;
		pVector sign(m.rot.x >= 0.f ? scaleFac : -scaleFac,
					m.rot.y >= 0.f ? scaleFac : -scaleFac,
					m.rot.z >= 0.f ? scaleFac : -scaleFac);
		pVector dif(( r.x - _abs(m.rot.x) ) * sign.x,
					( r.y - _abs(m.rot.y) ) * sign.x,
					( r.z - _abs(m.rot.z) ) * sign.x);
		m.rot	+= dif;
	}
}
